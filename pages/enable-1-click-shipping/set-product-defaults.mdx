...
Parcelcraft will now use the provided weight to calculate the total shipment weight for orders containing that product. The formula is (PRODUCT WEIGHT × PRODUCT QUANTITY) + PACKAGING WEIGHT = TOTAL WEIGHT. You will be entering the package weight on the [Carrier defaults](/enable-1-click-shipping/set-carrier-defaults) settings page.
