...
- Feature: Weight conversions are now more flesible with user-inputted metadata for the value of weight_unit.  Previously only once, pound, kilogram or gram were acceptible values for a weight unit.  Now values such as lbs, lb, lb., lbs., pnd, pounds, and pound are accptible.
- Fixed: The timeout for detecting an error in the loading of EasyPost carrier metadata was increased from 6 seconds to 10 seconds to accomodate real-world testing.
