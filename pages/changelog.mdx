...
- Breaking Change:  Ship status metadata for canceled shipents was changed from cancelled to canceled to be consistent with Stripe API spelling
- Feature: Users can now see an upshipped payments list rather than only an unshipped invoices list in default view settings
- Feature: Improved manage shipment and "reship" button when in Payment or Invoice view
- Feature: Added account health checkup to Parcelcraft setttings
- Feature: Added payment link permission and checkout session permissions to enable account compatibilty health check
