...
Our app will automatically add metadata to Stripe invoices prior to shipment.  If an invoice uses a shipping rate or if any product in the invoice is shippable, metadata with a key of "ship_status" and a value of "unshipped".  Invoices that are marked as unshipped will appear in your unshipped invoices list in Parcelcraft.
