...
Parcelcraft creates test and production labels that integrate directly into the EasyPost API.  We pre-prepare shipments by changing the metadata of invoices with a key of `ship_status` and a value of `unshipped` based on your product `shippable` status and if you have a `Shipping Rate` attached to your invoice.  If any product in your invoice is shippable, or if you have a shipping rate attached to your invoice, it will show up in your unshipped invoices list in the Parcelcraft Shipping app. Parcelcraft prefers `Stripe Invoices` data since this data they provides all the `Product` and `Shipment Rates` needed to calculate the domestic weight or the customs terms of your shipments. 