...
Parcelcraft crates test and production labels that integrate directly into the EasyPost API.  We also prepare shipments by changing the metadata of invoices with a key of `ship_status` and a value of `unshipped` to add them to your unshipped invoices list in the Parcelcraft Shipping app using the Stripe API. Parcelcraft prefers `Stripe Invoices` data since this data they provides all the `Product` and `Shipment Rates` needed to calculate the domestic weight or the customs terms of your shipments. 